#!/usr/bin/env bash
sudo docker run -t -d --name conangcc6-armv7hf conanio/conangcc8-armv7hf && \
...
sudo docker exec conangcc8-armv7hf conan install gtest/1.8.0@conanio/stable -s arch=armv7hf -s compiler=gcc -s compiler.version=7 -s compiler.libcxx=libstdc++ --build && \
