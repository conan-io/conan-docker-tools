...
if __name__ == "__main__":
    ...
    try:
        logging.info("Starting test: Simple")
        subprocess.check_call(["docker", "run", "-t", "-d", "-v", "%s:/tmp/project" % pwd, "--name", container, image])
        subprocess.check_call(["docker", "exec", container, "/bin/bash", "/tmp/project/test/simple/test_simple.sh"])
        logging.info("Test result (Simple): SUCCESS")
    except Exception as error:
        logging.error("Test result (Simple): FAILURE - {}".format(error))
        exit_code = 1
    finally:
