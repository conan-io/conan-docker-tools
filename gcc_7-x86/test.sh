#!/usr/bin/env bash
sudo docker run -t -d --name conangcc7-x86 lasote/conangcc7-x86 && \
...
sudo docker exec conangcc7-x86 conan install gtest/1.8.1@bincrafters/stable -s arch=x86 -s compiler=gcc -s compiler.version=7 -s compiler.libcxx=libstdc++ --build && \
