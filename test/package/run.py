...
if __name__ == "__main__":
    ...
    try:
        logging.info("Starting test: Package")
        ...
        subprocess.check_call(["docker", "exec", container, "/bin/bash", "/tmp/project/test/package/build_poco.sh"])
        subprocess.check_call(["docker", "exec", container, "/bin/bash", "/tmp/project/test/package/build_protobuf.sh"])
        logging.info("Test result (Package): SUCCESS")
    except Exception as error:
        logging.error("Test result (Package): FAILURE - {}".format(error))
        exit_code = 1
    finally:
